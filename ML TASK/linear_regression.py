...
def cost(x,y,w,b):
    m = x.shape[0]
    z = np.dot(x, w) + b
    cost = (1 / (2 * m)) * np.sum((z - y.reshape(-1)) ** 2)
    return cost
...
def plot_graphs(J_history, y_train, y_pred_train, y_test, y_pred_test):
    # Plot cost history
...
if __name__ == "__main__":
    ...
    x_train, x_test, y_train, y_test = data_split('Housing.csv')
    ...
    # Plot results
    plot_graphs(J_history, y_train, y_pred_train, y_test, y_pred_test)